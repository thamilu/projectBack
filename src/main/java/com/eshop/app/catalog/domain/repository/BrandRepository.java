...
public interface BrandRepository extends JpaRepository<Brand, Long> {
    ...
    default List<String> findExistingNames(List<String> names) {
        if (names == null || names.isEmpty())
            return List.of();
        List<String> lower = names.stream().map(s -> s == null ? null : s.toLowerCase()).filter(Objects::nonNull)
                .toList();
        return findNamesLowerCaseIn(lower);
    }

    @Query("SELECT new com.eshop.app.catalog.api.response.BrandSummaryResponse(b.id, b.name, b.slug, b.logoUrl) FROM Brand b WHERE b.active = true ORDER BY b.name")
    List<com.eshop.app.catalog.api.response.BrandSummaryResponse> findAllActiveBrandsSummary();
    ...
    default Page<Brand> searchWithCriteria(com.eshop.app.catalog.api.response.BrandSearchCriteria criteria,
            Pageable pageable) {
        if (criteria == null)
