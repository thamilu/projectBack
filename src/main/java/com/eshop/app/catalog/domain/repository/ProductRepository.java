...
import com.eshop.app.catalog.api.response.CategorySummaryResponse;
import com.eshop.app.catalog.domain.entity.Product;
import com.eshop.app.pricing.domain.repository.PriceStatsProjection;
...
public interface ProductRepository extends JpaRepository<Product, Long>, JpaSpecificationExecutor<Product> {
       ...
       @Query("SELECT p FROM Product p WHERE p.status = com.eshop.app.catalog.domain.entity.ProductStatus.ACTIVE AND p.deleted = false")
        ...
        */
       @Query("SELECT p FROM Product p WHERE p.deleted = false AND p.status = com.eshop.app.catalog.domain.entity.ProductStatus.ACTIVE")
        ...
        */
       @Query("""
                         SELECT new com.eshop.app.catalog.domain.repository.ProductDetailProjection(
        ...
        * CRITICAL-005 FIX: Pessimistic locking prevents race conditions in stock
        * updates
        */
       ...
       Page<Product> findByStatus(com.eshop.app.catalog.domain.entity.ProductStatus status, Pageable pageable);
       ...
       @Query("SELECT p FROM Product p WHERE p.store.city = :city AND p.status = com.eshop.app.catalog.domain.entity.ProductStatus.ACTIVE")
       Page<Product> findByStoreCity(@Param("city") String city, Pageable pageable);

       @Query("SELECT p FROM Product p WHERE p.store.state = :state AND p.status = com.eshop.app.catalog.domain.entity.ProductStatus.ACTIVE")
       Page<Product> findByStoreState(@Param("state") String state, Pageable pageable);

       @Query("SELECT p FROM Product p WHERE p.store.country = :country AND p.status = com.eshop.app.catalog.domain.entity.ProductStatus.ACTIVE")
       ...
       long countByStoreSellerProfileUserIdAndStatus(Long sellerId,
                     com.eshop.app.catalog.domain.entity.ProductStatus status);
       ...
       long countByStatus(com.eshop.app.catalog.domain.entity.ProductStatus status);
       ...
       long countByBrandIdAndStatus(Long brandId, com.eshop.app.catalog.domain.entity.ProductStatus status);
       ...
       default List<CategorySummaryResponse> findCategorySummariesByBrandId(Long brandId) {
              ...
              return rows.stream().map(r -> new CategorySummaryResponse(((Number) r[0]).longValue(), (String) r[1],
       ...
       default Map<Long, Long> countByBrandIds(Set<Long> ids) {
              ...
              return rows.stream().collect(
                            Collectors.toMap(r -> ((Number) r[0]).longValue(), r -> ((Number) r[1]).longValue()));
       }
        ...
        */
       @Query(value = "SELECT * FROM products WHERE search_vector @@ plainto_tsquery('english', :query) AND deleted = false", countQuery = "SELECT count(*) FROM products WHERE search_vector @@ plainto_tsquery('english', :query) AND deleted = false", nativeQuery = true)
