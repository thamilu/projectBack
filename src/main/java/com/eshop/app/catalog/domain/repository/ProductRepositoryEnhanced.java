...
public interface ProductRepositoryEnhanced extends JpaRepository<Product, Long> {
     ...
     * @throws org.springframework.dao.PessimisticLockingFailureException if lock
     *                                                                    cannot be
     *                                                                    acquired
     */
    ...
    @Query("""
            SELECT new map(
                ...
                COUNT(CASE WHEN p.status = com.eshop.app.catalog.domain.entity.ProductStatus.ACTIVE THEN 1 END) as activeProducts,
