...
import com.eshop.app.catalog.application.mapper.BrandMapper;
import com.eshop.app.catalog.application.port.in.BrandUseCase;
...
public class BrandUseCaseImpl implements BrandUseCase {
    ...
    public List<BrandResponse> createBrandsBatch(List<BrandRequest> requests) {
        ...
        if (!existing.isEmpty()) {
            throw new DuplicateResourceException("Brands already exist: " + String.join(", ", existing));
        }
    ...
    public BrandDetailResponse getBrandDetailById(Long id) {
        ...
        resp.setActiveProductCount(productRepository.countByBrandIdAndStatus(id,
                com.eshop.app.catalog.domain.entity.ProductStatus.ACTIVE));
    ...
    public BrandResponse updateBrand(Long id, BrandRequest request) {
        ...
        if (!brand.getName().equalsIgnoreCase(request.getName())) {
            String newSlug = StringUtils.hasText(request.getSlug()) ? request.getSlug()
                    : SlugUtils.generateSlug(request.getName());
    ...
    public void deleteBrand(Long id) {
        ...
        if (productCount > 0) {
            throw new ConflictException(
                    "Cannot delete brand '" + brand.getName() + "'. It has " + productCount + " associated products.");
        }
    ...
    public BrandResponse restoreBrand(Long id) {
        Brand brand = brandRepository.findByIdIncludeDeleted(id)
