...
import com.eshop.app.core.api.request.BatchDeleteRequest;
 ...
 * 
 * <p>
 * Enterprise-grade CRUD operations for products with:
 * <ul>
 ...
 * @since 1.0.0
 */
@io.swagger.v3.oas.annotations.tags.Tag(name = "Products", description = "Product management endpoints - CRUD operations, search, filtering")
...
public class ProductController extends BaseController {
     ...
     * - Rate limiting: Configured via @RateLimiter annotations on sensitive
     * endpoints
     * - Input validation: Applied via @Valid, @Positive, etc. on all DTOs and
     * parameters
     * - Authorization: RBAC checks via @PreAuthorize expressions for role and
    ...
    @Operation(summary = "Create new product", description = """
            ...
            **Notes:**
            ...
            """, security = @SecurityRequirement(name = "Bearer Authentication"), requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(required = true, description = "Product creation payload", content = @Content(schema = @Schema(implementation = com.eshop.app.catalog.api.request.ProductCreateRequest.class))), responses = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "201", description = "Product created successfully", content = @Content(schema = @Schema(implementation = com.eshop.app.catalog.api.response.ProductResponse.class))),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "Invalid input data"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "409", description = "Product with SKU already exists")
    })
    ...
    @Operation(summary = "Batch create products", description = "Create multiple products in a single request (max 100 products). Business logic handled in service layer with comprehensive error tracking.", security = @SecurityRequirement(name = "Bearer Authentication"))
    ...
    public ResponseEntity<ApiResponse<ProductResponse>> updateProduct(
        ...
        if (ifMatch != null) {
            ...
            String currentEtag = etagGenerator.forTimestampedEntity(
                    current.id(),
                    current.updatedAt() != null ? current.updatedAt().atZone(java.time.ZoneOffset.UTC).toInstant()
                            : null);
    ...
    public ResponseEntity<ApiResponse<ProductResponse>> updateStock(
        ...
        String userId = extractUserId(jwt);
        log.info("Updating stock for product {}: {} {} by user {}", id, request.operation(), request.quantity(),
                userId);
    ...
    @DeleteMapping("/batch")
    @PreAuthorize("hasRole(@appProperties.security.roles.admin)")
    @Operation(summary = "Batch delete products", description = "Delete multiple products (max 100). Business logic in service layer with detailed error tracking.", security = @SecurityRequirement(name = "Bearer Authentication"))
    ...
    @GetMapping("/{id}")
    @Operation(summary = "Get product by ID", description = "Retrieve product details by ID. Supports conditional requests via If-None-Match header for efficient caching.")
    @ApiResponses({
    ...
    public ResponseEntity<ApiResponse<ProductResponse>> getProductById(
        ...
        String etag = etagGenerator.forTimestampedEntity(
                response.id(),
                response.updatedAt() != null ? response.updatedAt().atZone(java.time.ZoneOffset.UTC).toInstant()
    ...
    @GetMapping("/sku/{sku}")
    @Operation(summary = "Get product by SKU", description = "Retrieve product details by SKU code")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Product found"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "Product not found")
    })
    ...
    @Operation(summary = "Get product by SEO URL", description = "Retrieve product by SEO-friendly URL slug")
    ...
    @GetMapping
    @Operation(summary = "Get all products", description = "Retrieve paginated list of all active products. Automatic pagination validation applied.")
    ...
    @Operation(summary = "Advanced product search", description = "Search products with keyword filtering. Rate limited to prevent abuse.", security = @SecurityRequirement(name = "Bearer Authentication"))
    @ApiResponses({
    ...
    @Operation(summary = "Get products by brand", description = "Retrieve all products from a specific brand")
    ...
    @Operation(summary = "Get products by store", description = "Retrieve all products from a specific seller's store")
    ...
    @GetMapping("/featured")
    @Operation(summary = "Get featured products", description = "Retrieve featured/promoted products with pagination")
    ...
    @RateLimiter(name = "search")
    @Operation(summary = "Full-text product search", description = "Full-text search using PostgreSQL tsvector for name, description, and SKU. Rate limited to prevent abuse.", security = @SecurityRequirement(name = "Bearer Authentication"))
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Search results"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "429", description = "Too many requests")
    })
