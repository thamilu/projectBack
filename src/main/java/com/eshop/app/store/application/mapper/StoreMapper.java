package com.eshop.app.store.application.mapper;
...
public interface StoreMapper {
    ...
    @Mapping(target = "isVerified", expression = "java(store.getSellerProfile() != null && store.getSellerProfile().getStatus() == com.eshop.app.seller.shared.domain.enums.SellerStatus.ACTIVE)")
    ...
    Store toStore(com.eshop.app.user.domain.entity.SellerProfile profile);

    default boolean syncMissingData(com.eshop.app.store.domain.entity.Store store,
            com.eshop.app.user.domain.entity.SellerProfile profile) {
        if (profile == null || store == null)
            return false;
        boolean changed = false;

        if (isBlank(store.getAddressLine1()) && !isBlank(profile.getStoreAddressLine1())) {
            store.setAddressLine1(profile.getStoreAddressLine1());
            changed = true;
        } else if (isBlank(store.getAddressLine1()) && !isBlank(profile.getAddressLine1())) {
            store.setAddressLine1(profile.getAddressLine1());
            changed = true;
        }

        if (isBlank(store.getCity()) && !isBlank(profile.getStoreCity())) {
            store.setCity(profile.getStoreCity());
            changed = true;
        } else if (isBlank(store.getCity()) && !isBlank(profile.getCity())) {
            store.setCity(profile.getCity());
            changed = true;
        }

        if (isBlank(store.getState()) && !isBlank(profile.getStoreState())) {
            store.setState(profile.getStoreState());
            changed = true;
        } else if (isBlank(store.getState()) && !isBlank(profile.getState())) {
            store.setState(profile.getState());
            changed = true;
        }

        if (isBlank(store.getDistrict()) && !isBlank(profile.getStoreDistrict())) {
            store.setDistrict(profile.getStoreDistrict());
            changed = true;
        } else if (isBlank(store.getDistrict()) && !isBlank(profile.getDistrict())) {
            store.setDistrict(profile.getDistrict());
            changed = true;
        }

        if (isBlank(store.getTaluk()) && !isBlank(profile.getStoreTaluk())) {
            store.setTaluk(profile.getStoreTaluk());
            changed = true;
        } else if (isBlank(store.getTaluk()) && !isBlank(profile.getTaluk())) {
            store.setTaluk(profile.getTaluk());
            changed = true;
        }

        if (isBlank(store.getShopHandle()) && !isBlank(profile.getShopHandle())) {
            store.setShopHandle(profile.getShopHandle());
            changed = true;
        }
        if (isBlank(store.getLogoUrl()) && !isBlank(profile.getShopLogoUrl())) {
            store.setLogoUrl(profile.getShopLogoUrl());
            changed = true;
        }
        if (isBlank(store.getPhone()) && !isBlank(profile.getBusinessMobileNumber())) {
            store.setPhone(profile.getBusinessMobileNumber());
            changed = true;
        }

        if (isBlank(store.getPostalCode()) && !isBlank(profile.getStorePincode())) {
            store.setPostalCode(profile.getStorePincode());
            changed = true;
        } else if (isBlank(store.getPostalCode()) && !isBlank(profile.getPincode())) {
            store.setPostalCode(profile.getPincode());
            changed = true;
        }
    ...
    default String combineAddress(Store store) {
        ...
        com.eshop.app.user.domain.entity.SellerProfile profile = store.getSellerProfile();

        String addr1 = !isBlank(store.getAddressLine1()) ? store.getAddressLine1()
                : (profile != null ? profile.getStoreAddressLine1() : null);
        String addr2 = !isBlank(store.getAddressLine2()) ? store.getAddressLine2()
                : (profile != null ? profile.getStoreAddressLine2() : null);
        String city = !isBlank(store.getCity()) ? store.getCity() : (profile != null ? profile.getStoreCity() : null);
        String taluk = !isBlank(store.getTaluk()) ? store.getTaluk()
                : (profile != null ? profile.getStoreTaluk() : null);
        String state = !isBlank(store.getState()) ? store.getState()
                : (profile != null ? profile.getStoreState() : null);
        String pincode = !isBlank(store.getPostalCode()) ? store.getPostalCode()
                : (profile != null ? profile.getStorePincode() : null);
