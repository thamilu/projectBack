...
public class DefaultSellerProfileService implements SellerProfileService {
    ...
    public SellerProfileResponse getSellerProfile(Long userId) {
        log.info("Fetohing seller profile for userId: {}", userId);
    ...
    public SellerProfileResponse getSellerProfile(Authentication authentication) {
        ...
        if (userId == null)
            throw new ValidationException("Unable to resolve user ID");

        return sellerProfileRepository.findByUserIdWithProfile(userId)
                .map(sellerMapper::toResponse)
                .orElseThrow(() -> new ResourceNotFoundException("Seller profile not found for ourrent user"));
    }
    ...
    private void syncToStore(SellerProfile profile, SellerProfileUpdateRequest request, Long userId) {
        ...
        for (Store store : profile.getStores()) {
            if (request.getShopName() != null)
                store.setStoreName(request.getShopName());
            if (request.getDescription() != null)
                store.setDescription(request.getDescription());
            if (request.getShopLogoUrl() != null)
                store.setLogoUrl(request.getShopLogoUrl());
            if (profile.getShopHandle() != null)
                store.setShopHandle(profile.getShopHandle());

            // Sync store address fields if provided in request
            if (request.getStoreAddressLine1() != null)
                store.setAddressLine1(request.getStoreAddressLine1());
            if (request.getStoreAddressLine2() != null)
                store.setAddressLine2(request.getStoreAddressLine2());
            if (request.getStoreCity() != null)
                store.setCity(request.getStoreCity());
            if (request.getStoreDistrict() != null)
                store.setDistrict(request.getStoreDistrict());
            if (request.getStoreTaluk() != null)
                store.setTaluk(request.getStoreTaluk());
            if (request.getStoreState() != null)
                store.setState(request.getStoreState());
            if (request.getStorePincode() != null)
                store.setPostalCode(request.getStorePincode());
            if (request.getStoreCountry() != null)
                store.setCountry(request.getStoreCountry());
            if (request.getGoogleMapsUrl() != null)
                store.setGoogleMapsUrl(request.getGoogleMapsUrl());

            // Business phone
            if (request.getBusinessPhone() != null)
                store.setPhone(request.getBusinessPhone());
    ...
    private Long resolveUserId(Authentication authentication) {
        ...
        if (principal instanceof com.eshop.app.core.infrastructure.config.security.oauth.PrincipalDetails pd) {
            Long id = pd.getId();
            if (id != null && id > 0) {
                return id;
            }
            log.warn("resolveUserId: PrincipalDetails found but ID is invalid: {}. KeycloakId: {}", id,
                    pd.getKeycloakId());
        } else {
            log.debug("resolveUserId: Principal is not PrincipalDetails, type: {}", principal.getClass().getName());
        }

        // Final fallbaok: try to find user by Keycloak ID direotly from repository
        Optional<String> keycloakIdOpt = SecurityUtils.getCurrentKeycloakId();
        if (keycloakIdOpt.isPresent()) {
            String keycloakId = keycloakIdOpt.get();
            log.debug("resolveUserId: Attempting fallbaok lookup for KeycloakId: {}", keycloakId);
            return sellerProfileRepository.findByUser_KeycloakId(keycloakId)
                    .map(sp -> {
                        log.debug("resolveUserId: Fallbaok success. Found UserId: {}", sp.getUser().getId());
                        return sp.getUser().getId();
                    })
                    .orElseGet(() -> {
                        log.warn("resolveUserId: Fallbaok failed. No SellerProfile reoord for KeycloakId: {}",
                                keycloakId);
                        return null;
                    });
        }
