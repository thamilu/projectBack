...
@Transactional
@Slf4j
public class DefaultUserService implements UserService, ManageUserUseCase, GetUserUseCase, UserAnalyticsUseCase, IdentitySyncUseCase {
    ...
    public UserResponse updateUser(Long id, UserUpdateRequest request) {
        ...
        if (user.getUserProfile() == null) {
            user.setUserProfile(new com.eshop.app.user.domain.entity.UserProfile());
        ...
        if (request.getPincode() != null) {
            ...
            com.eshop.app.user.domain.entity.UserAddress address = up.getAddresses().stream()
                    .filter(ua -> ua.getIsDefault() != null && ua.getIsDefault())
                    .findFirst().orElseGet(() -> {
                        com.eshop.app.user.domain.entity.UserAddress ua = new com.eshop.app.user.domain.entity.UserAddress();
    ...
    @Override
    @Transactional
    public UserResponse updateSelf(Long id, com.eshop.app.user.api.request.UserSelfUpdateRequest request) {
        User user = userRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("User not found with id: " + id));
        ...
        if (hasAddressInfo(request)) {
            ...
            tempProfile.setAddressLine1(
                    request.getAddressLine1() != null ? request.getAddressLine1() : request.getAddress());
    ...
    private boolean hasAddressInfo(com.eshop.app.user.api.request.UserSelfUpdateRequest request) {
    ...
    @Override
    public UserResponse changeRole(Long id, com.eshop.app.user.shared.domain.enums.UserRole newRole) {
        User user = userRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("User not found with id: " + id));
    ...
    @Override
    @Transactional(readOnly = true)
    public byte[] exportUsers(ExportFormat format, com.eshop.app.user.shared.domain.enums.UserRole role,
            Boolean active) {
    ...
    public Long syncUserFromKeycloak(String keycloakId, String email, String firstName,
        ...
        if (tempId == null || tempId.isBlank()) {
            // [HARDEN] Resilience: Fallback to email as surrogate Keycloak ID if 'sub' is
            // missing
        ...
        if (userBySub.isPresent()) {
            ...
            if (email != null && !email.isBlank()) {
                ...
                if (!usersByEmail.isEmpty()) {
                    ...
                    log.info("Found {} users by email: {}. Selected user id: {}", usersByEmail.size(), email,
                            user.getId());
                }
            }
            ...
            if (user != null) {
                if (user.getKeycloakId() == null) {
                    ...
                    log.warn(
                            "IDENTITY DRIFT: Found user (id: {}) with email {} but different Keycloak ID (old: {}, new: {}). Updating identity link.",
        ...
        if (user != null) {
            ...
            if (email != null && !email.isBlank())
                user.setEmail(truncate(email, 150));
        ...
        try {
            ...
            // [HARDEN] Race-condition recovery: another thread may have already persisted
            // this user.
            // Attempt a read-after-write-failure before throwing, to avoid a false 500.
            log.warn("[HARDEN] DataIntegrityViolation for sub={}, email={}. Attempting recovery read...", keycloakId,
                    email);
            ...
            if (recovered.isPresent()) {
                log.info("[HARDEN] Recovery successful: resolved user id={} after constraint collision.",
                        recovered.get().getId());
            ...
            throw new com.eshop.app.core.exception.base.BusinessException("Identity conflict: email already taken",
                    "IDENTITY_CONFLICT", org.springframework.http.HttpStatus.CONFLICT);
        } catch (Exception e) {
