...
/**
 * 🛡️ Hardened Consolidated Security Configuration
 * 
 * Unified security policy for the E-Shop platform.
 * Supports dual-realm validation (Admin/User) and automatic user identity syncing.
 */
@Configuration
@EnableWebSecurity
...
public class SecurityConfig {
    ...
    public SecurityFilterChain staticResourcesChain(HttpSecurity http) throws Exception {
        http
            .securityMatcher("/favicon.ico", "/static/**", "/public/**", "/error", "/css/**", "/js/**", "/images/**")
    ...
    /**
     * 🔐 Chain 1: ADMIN API -> Validates with 'eshop-admin' realm if configured
     */
    @Bean
    @Order(1)
    public SecurityFilterChain adminSecurityFilterChain(HttpSecurity http) throws Exception {
        if (adminRealmIssuer == null || adminRealmIssuer.isBlank()) {
            log.warn("⚠️ Admin Realm Issuer not configured. Admin chain will be inactive.");
            return null;
        }

        log.info("🛡️ Configuring ADMIN Security Chain (Issuer: {})", adminRealmIssuer);

        http
            .securityMatcher("/api/admin/**", "/api/v1/admin/**", "/api/v1/sellers/requests/**", "/api/v1/dashboard/admin/**")
            .csrf(AbstractHttpConfigurer::disable)
            .cors(cors -> cors.configurationSource(corsConfigurationSource()))
            .sessionManagement(s -> s.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
            .authorizeHttpRequests(auth -> auth.anyRequest().hasRole(appProperties.getSecurity().getRoles().getAdmin()))
            .oauth2ResourceServer(oauth2 -> oauth2
                .jwt(jwt -> jwt
                    .decoder(NimbusJwtDecoder.withIssuerLocation(adminRealmIssuer).build())
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        log.info("🛡️ Configuring PRIMARY Security Chain (Issuer: {})", userRealmIssuer);
        ...
        http
            ...
            .authorizeHttpRequests(auth -> auth
                // Public endpoints
                .requestMatchers("/api/v1/public/**", "/api/v1/auth/**", "/api/v1/csp/**", "/api/v1/locations/**", "/swagger-ui/**", "/v3/api-docs/**", "/error").permitAll()
                .requestMatchers(HttpMethod.GET, "/api/v1/products/**", "/api/v1/categories/**", "/api/v1/brands/**", "/api/v1/stores/**").permitAll()
            ...
            .oauth2ResourceServer(oauth2 -> oauth2
                .jwt(jwt -> jwt
                    .decoder(NimbusJwtDecoder.withIssuerLocation(userRealmIssuer).build())
                    .jwtAuthenticationConverter(jwtAuthenticationConverter())
                )
                .authenticationEntryPoint(authenticationEntryPoint())
                .accessDeniedHandler(accessDeniedHandler())
    ...
    public org.springframework.core.convert.converter.Converter<Jwt, org.springframework.security.authentication.AbstractAuthenticationToken> jwtAuthenticationConverter() {
        log.debug("🔧 Initializing JWT Converter with Identity Sync");
    ...
    private Long syncUserIdentity(Jwt jwt, Set<String> roles) {
        ...
        try {
            ...
            if (userService != null) {
                ...
                Long id = userService.syncUserFromKeycloak(keycloakId, email, firstName, lastName, phone, emailVerified);
                userService.syncUserRoles(id, roles);
                return id;
            }
        } catch (Exception e) {
            log.error("🚨 CRITICAL: Identity sync failed for Keycloak user {} (email: {}): {}. This will cause 500 errors in downstream services.", 
    ...
    public AuthenticationEntryPoint authenticationEntryPoint() {
        return (request, response, ex) -> {
            log.error("❌ Authentication error on {}: {}", request.getRequestURI(), ex.getMessage());
    ...
    public AccessDeniedHandler accessDeniedHandler() {
        return (request, response, ex) -> {
            ...
            log.warn("🚫 [ACCESS_DENIED] {} {} | principal={} | reason={}",
                request.getMethod(), request.getRequestURI(), principal, ex.getMessage());
            response.setStatus(HttpServletResponse.SC_FORBIDDEN);
            response.setContentType(MediaType.APPLICATION_JSON_VALUE);
            ApiError error = ApiError.of(403, "Forbidden", "You do not have permission to perform this action", request.getRequestURI());
            objectMapper.writeValue(response.getOutputStream(), error);
        };
    }
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        config.setAllowedHeaders(Arrays.asList("Authorization", "Content-Type", "X-Requested-With", "X-Request-ID", "X-Idempotency-Key", "Accept", "Origin"));
