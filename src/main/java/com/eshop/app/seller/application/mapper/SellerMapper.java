...
public interface SellerMapper {
    ...
    // Address Fallbacks logic handled via custom after-mapping if needed, but
    // simple ones here:
    ...
    @AfterMapping
    default void handleStoreFallbacks(SellerProfile profile,
            @MappingTarget SellerProfileResponse.SellerProfileResponseBuilder response) {
        Store firstStore = (profile.getStores() != null && !profile.getStores().isEmpty())
                ? profile.getStores().iterator().next()
                : null;

        response.storeAddressLine1(profile.getStoreAddressLine1() != null ? profile.getStoreAddressLine1()
                : (firstStore != null ? firstStore.getAddressLine1() : profile.getAddressLine1()));
        response.storeCity(profile.getStoreCity() != null ? profile.getStoreCity()
                : (firstStore != null ? firstStore.getCity() : profile.getCity()));
        response.storeDistrict(profile.getStoreDistrict() != null ? profile.getStoreDistrict()
                : (firstStore != null ? firstStore.getDistrict() : profile.getDistrict()));
        response.storeTaluk(profile.getStoreTaluk() != null ? profile.getStoreTaluk()
                : (firstStore != null ? firstStore.getTaluk() : profile.getTaluk()));
        response.storeState(profile.getStoreState() != null ? profile.getStoreState()
                : (firstStore != null ? firstStore.getState() : profile.getState()));
        response.storePincode(profile.getStorePincode() != null ? profile.getStorePincode()
                : (firstStore != null ? firstStore.getPostalCode() : profile.getPincode()));
        response.googleMapsUrl(profile.getGoogleMapsUrl() != null ? profile.getGoogleMapsUrl()
