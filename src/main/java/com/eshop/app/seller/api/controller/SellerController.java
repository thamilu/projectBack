 ...
 * 
 * <p>
 * Supports step-wise seller onbcarding with modular architecture:
 * SellerProfile â†’ SellerKYC, SellerDocument, SellerBankAccount,
...
public class SellerController {
  ...
  private final com.eshop.app.user.application.service.UserService userService;

  /**
   * Register a new seller profile.
   */
  // [HARDEN] Authorization is handled by SecurityConfig filter chain:
  // POST /api/v1/sellers/register â†’ .authenticated()
  // Removing @PreAuthorize SpEL bean reference to prevent 403 on unresolved
  // @appProperties
  ...
  public ResponseEntity<ApiResponse<Void>> rejectSeller(
      @PathVariable Long id,
      @Valid @RequestBody com.eshop.app.seller.api.request.RejectionRequest request,
      @Parameter(hidden = true) Authentication authentication) {
  ...
  public ResponseEntity<ApiResponse<java.util.List<com.eshop.app.seller.api.response.SellerIdentityTypeResponse>>> getIdentityTypes() {
    java.util.List<com.eshop.app.seller.api.response.SellerIdentityTypeResponse> types = java.util.Arrays
        .stream(com.eshop.app.seller.shared.domain.enums.SellerIdentityType.values())
        .map(type -> com.eshop.app.seller.api.response.SellerIdentityTypeResponse.builder()
            .type(type.name())
  ...
  public ResponseEntity<ApiResponse<java.util.List<com.eshop.app.seller.api.response.SellerBusinessTypeResponse>>> getBusinessTypes() {
    java.util.List<com.eshop.app.seller.api.response.SellerBusinessTypeResponse> types = java.util.Arrays
        .stream(com.eshop.app.seller.shared.domain.enums.SellerBusinessType.values())
        .map(type -> com.eshop.app.seller.api.response.SellerBusinessTypeResponse.builder()
            .type(type.name())
            .label(type.getDisplayName())
            .build())
        .collect(java.util.stream.Collectors.toList());
