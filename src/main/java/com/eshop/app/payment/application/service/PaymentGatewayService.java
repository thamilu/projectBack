...
public class PaymentGatewayService {
    ...
    private PaymentGatewayResult processStripePayment(PaymentRequest request, Payment payment) {
        ...
        try {
            ...
            stripeParams.put("amount", request.getAmount().multiply(BigDecimal.valueOf(100)).intValue()); // Convert to
                                                                                                          // cents
    ...
    private PaymentGatewayResult processRazorpayPayment(PaymentRequest request, Payment payment) {
        ...
        try {
            ...
            razorpayParams.put("amount", request.getAmount().multiply(BigDecimal.valueOf(100)).intValue()); // Convert
                                                                                                            // to paise
    ...
    private boolean isUpiPayment(PaymentMethod method) {
    ...
    private boolean isWalletPayment(PaymentMethod method) {
        return method != null && method.name().contains("WALLET");
    }

    private String getWalletProvider(PaymentMethod method) {
    ...
    private String detectCardBrand(String cardNumber) {
        ...
        if (number.startsWith("6"))
            return "DISCOVER";
        if (number.startsWith("508") || number.startsWith("60"))
            return "RUPAY";
