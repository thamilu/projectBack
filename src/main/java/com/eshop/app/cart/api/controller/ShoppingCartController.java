 ...
 * 
 * Features:
 * - Anonymous Cart Management: Guest users can create and manage carts without
 * authentication
 * - Authenticated Cart Operations: Logged-in users have persistent cart
 * management
...
@io.swagger.v3.oas.annotations.tags.Tag(name = "Shopping Cart", description = "Shopping cart management with anonymous and authenticated support")
public class ShoppingCartController {
    ...
    @PostMapping("/cart")
    @Operation(summary = "Create new anonymous cart", description = "Create a new shopping cart for anonymous users. Returns unique cart code.")
    ...
    @Operation(summary = "Get cart contents by code", description = "Retrieve cart contents using cart code (anonymous) or user authentication")
    ...
    @Operation(summary = "Update entire cart", description = "Update cart with new items (replaces existing items)")
    ...
    @Operation(summary = "Add single product to cart", description = "Add a single product to cart by SKU")
    ...
    @Operation(summary = "Add multiple products to cart", description = "Add multiple products to cart in a single operation")
    ...
    @Operation(summary = "Remove product from cart", description = "Remove a product from cart by SKU")
    ...
    @Operation(summary = "Checkout anonymous cart", description = "Convert anonymous cart to order without authentication")
    public ResponseEntity<ApiResponse<OrderResponse>> checkoutAnonymousCart(
            @Parameter(description = "Cart code") @PathVariable String code,
            @Valid @RequestBody CheckoutRequest request) {
        OrderResponse response = checkoutUseCase.checkoutAnonymousCart(code, request);
        return ResponseEntity
                .status(HttpStatus.CREATED)
    ...
    public ResponseEntity<ApiResponse<OrderResponse>> checkoutAuthenticatedCart(
            @Parameter(description = "Cart code") @PathVariable String code,
            @Valid @RequestBody CheckoutRequest request) {
        OrderResponse response = checkoutUseCase.checkoutAuthenticatedCart(code, request);
        return ResponseEntity
                .status(HttpStatus.CREATED)
                .body(ApiResponse.success("Order created successfully", response));
    }
