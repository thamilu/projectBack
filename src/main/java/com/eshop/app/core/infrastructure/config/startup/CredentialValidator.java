 ...
 * 
 * <p>
 * Validates that critical secrets and credentials are properly configured
...
public class CredentialValidator {
    ...
    public void validateCredentials() {
        log.info("🔐 Starting credential validation for profile: {} (enforceValidation={})", activeProfile,
                enforceValidation);

        if (!enforceValidation || (activeProfile != null && activeProfile.contains("test"))) {
            log.info("Skipping credential validation (enforceValidation={} and activeProfile={})", enforceValidation,
                    activeProfile);
            return;
        }
